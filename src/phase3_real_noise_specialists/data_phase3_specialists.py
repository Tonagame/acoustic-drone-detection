...
class RealNoiseSpecialistDataset(Dataset):
    """Fixed-view real-noise training dataset for one specialist CNN."""
    ...
    def _rng(self, idx: int):
        if self.augment:
            return random
        return random.Random(self.seed + idx * 104729 + self.view_idx * 1009)
    ...
    def __getitem__(self, idx: int):
        ...
        view = self.preproc.create_audio_views(audio)[self.view_idx]
        logmel = self.preproc.audio_to_logmel(view)
